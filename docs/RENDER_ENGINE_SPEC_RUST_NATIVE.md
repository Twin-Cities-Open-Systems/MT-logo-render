...
2) mark-<...>   (always; default mark-none)
3) badge-<...>  (always; default badge-none)
4) label-<...>  (only if present after sanitization)
5) glyph-<...>  (only if present and renderable; encoded as UXXXXXXXX[_UYYYYYYYY...])
6) font-<hash8> (only if PNG uses user font)
